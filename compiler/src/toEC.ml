...
let get_aux ?(fresh = true) env prefix ty =
  ...
  if (not fresh) && 0 < List.length l then begin
    List.last l
  end else if i < List.length l then begin
    List.nth l i
  end else begin
...
module EcLeakConstantTime(EE: EcExpression): EcLeakage = struct
  ...
  let ec_fun_leak_init env = reset_leak (leakacc ~fresh:false env)
  ...
  let leakv_call env = get_aux ~fresh:false env "leak_c" "JLeakage.leakage"
