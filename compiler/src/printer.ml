...
let rec pp_gmitem rr pp_len pp_opn pp_var fmt =
  function
  | MdItem it -> pp_pitem pp_len pp_opn pp_var fmt it
  | MdFunctor fd ->
    F.fprintf fmt "@[<v>module %s @[%a@] {@ @[<v>%a@]@ }@]"
      fd.functorname
      (pp_gmparams pp_var pp_len) fd.functorparams
      (pp_gmprog rr pp_len pp_opn pp_var) fd.functorbody
  | MdModApp fa ->
    F.fprintf fmt "@[<v>module %s = %s @[%a@]; @]"
      fa.ma_name fa.ma_func
      (pp_list ",@ " (pp_gmarg pp_var pp_len)) fa.ma_args

and pp_gmprog rr pp_len pp_opn pp_var fmt p =
  let p = if rr then List.rev p else p in
  F.fprintf fmt "@[<v>%a@]"
    (pp_list "@," (pp_gmitem rr pp_len pp_opn pp_var))
    p
